...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete("/api/users/:id", requireAuth, requirePermission("canManageUsers"), async (req: any, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
  ...
  app.delete("/api/meetings/:id", requireAuth, requirePermission("canCreateMeetings"), async (req: any, res: Response) => {
    try {
      ...
      if (isNaN(meetingId)) {
        return res.status(400).json({ message: "ID de réunion invalide" });
      }
      ...
      // Vérifier que l'utilisateur peut supprimer cette réunion
      const userId = req.user.id;
      const userPermissions = req.user.permissions;
      
      if (meeting.createdBy !== userId && !userPermissions.canManageUsers) {
        return res.status(403).json({ 
          message: "Vous n'avez pas les droits pour supprimer cette réunion" 
        });
      }

      await storage.deleteMeeting(meetingId);
      console.log(`[API] Meeting ${meetingId} deleted by user ${userId}`);
